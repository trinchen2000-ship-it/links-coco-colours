...
// ========== LANDING PAGE ==========
// Suche + Berater: kein Caching, immer frisch
app.get('/suche', (req, res) => {
  res.setHeader('Cache-Control','no-store, no-cache, must-revalidate');
  res.setHeader('Pragma','no-cache');
  res.setHeader('Expires','0');
  res.sendFile(path.join(__dirname, 'public', 'suche.html'), {etag:false,lastModified:false});
});
app.get('/berater', (req, res) => {
  res.setHeader('Cache-Control','no-store, no-cache, must-revalidate');
  res.sendFile(path.join(__dirname, 'public', 'berater.html'), {etag:false,lastModified:false});
});
